...
export class NodeAggregationSelectionSet extends QueryASTNode {
    ...
    public getSubqueries(relatedNode: Cypher.Node): Cypher.Clause[] {
        ...
        return [...this.fields].flatMap((f) => f.getSubqueries(relatedNode));
    ...
    public getProjectionFields(variable: Cypher.Variable): Record<string, Cypher.Expr>[] {
        ...
        const nodeFieldProjections = this.fields.flatMap((f) => f.getProjectionFields(variable));
